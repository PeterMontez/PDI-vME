...
List<RGB> kmeans((Bitmap bmp, float[] img) t)
{
    byte[] rbytes = new byte[768];
    int hei = t.bmp.Height;
    int wid = t.bmp.Width;
    double limit = 50;
    float[] Img = t.img;
    float[] newImg = new float[Img.Length];
    List<Color> fullimage = new List<Color>();
    List<RGB> kValues = new List<RGB>();
    Random random = new Random();
    random.NextBytes(rbytes);
    double maxdist = 0;
    List<Close> pontosProximos = new List<Close>();
    
    for (int i = 0; i < rbytes.Length; i+=3)
    {
        RGB pixel = new RGB(rbytes[i], rbytes[i+1], rbytes[i+2]);
        kValues.Add(pixel);
    }
    ...
    for (int j = 0; j < hei; j++)
    {
        for (int i = 0; i < wid; i++)
        {
            fullimage.Add(t.bmp.GetPixel(i,j));
        }
    }

    foreach (var valork in kValues)
    {
        Close pertinho = new Close();
        pontosProximos.Add(pertinho);
    }
    ...
    while (true)
    {
        ...
        for (int j = 0; j < fullimage.Count; j++)
        {
            int closestindex = 0;
            ...
            {
                double valueR = Math.Abs(fullimage[j].R - kValues[i].R);
                double valueG = Math.Abs(fullimage[j].G - kValues[i].G);
                double valueB = Math.Abs(fullimage[j].B - kValues[i].B);

                double dist = Math.Sqrt((Math.Pow(valueR, 2)) + (Math.Pow(valueG, 2)) + (Math.Pow(valueB, 2)));
            ...
            pontosProximos[closestindex].Adicionar(fullimage[j]);
            pontosProximos[closestindex].AdicionarIndex(j);

        }

        for (int i = 0; i < pontosProximos.Count; i++)
        {
            
            int points = (pontosProximos[i]).CloseColor.Count;
            double R = 0;
            double G = 0;
            double B = 0;

            foreach (var value in pontosProximos[i].CloseColor)
            {
                R += value.R;
                G += value.G;
                B += value.B;
            ...
            double valueG = Math.Abs(G - kValues[i].G);
            double valueB = Math.Abs(B - kValues[i].B);
            double valueR = Math.Abs(R - kValues[i].R);
...
byte[] NewKMeans(Bitmap bmp, float[] img)
{
    // Vetor resultado
    ...
    for (int x = 0; x < bmp.Width; x++)
    {
        for (int y = 0; y < bmp.Height; y++)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                dist += Math.Pow(rgbValues[i] - img[i], 2);
            }
            dist = (float)Math.Sqrt(dist);

            result[x + y * bmp.Width] = (byte)dist;
        }
    }
...
(Bitmap bmp, float[] img) interpolate((Bitmap bmp, float[] img) t)
{
    ...
    for (int j = 5; j < hei-5; j++)
    {
        for (int i = 5; i < wid-5; i++)
        {
            int index = i + j * wid;
            ...
            if (Img[index] == 0f)
            {
                ...
                if (Img[c1Index] == 0f || Img[c2Index] == 0f || Img[c3Index] == 0f || Img[c4Index] == 0f)
                {
                    while (true)
                    {
                        if (count % 2 == 0)
                        {
                            c1Index = i - (1 - (int)(count/2)) + (j - 1 - ((int)(count/2))-1) * wid;
                            c2Index = i + (1 + (int)(count/2)) + (j - 1 - ((int)(count/2))-1) * wid;
                            c3Index = i - (1 - (int)(count/2)) + (j + 1 + ((int)(count/2))+1) * wid;
                            c4Index = i + (1 + (int)(count/2)) + (j + 1 + ((int)(count/2))+1) * wid;
                        }
                        else
                        {
                            c1Index = i - (1 - (int)(count/2)-1) + (j - (1 - (int)(count/2))-1) * wid;
                            c2Index = i + (1 + (int)(count/2)+1) + (j - (1 - (int)(count/2))-1) * wid;
                            c3Index = i - (1 - (int)(count/2)-1) + (j + (1 + (int)(count/2))+1) * wid;
                            c4Index = i + (1 + (int)(count/2)+1) + (j + (1 + (int)(count/2))+1) * wid;
                        }
                        if (Img[c1Index] > 0f && Img[c2Index] > 0f && Img[c3Index] > 0f && Img[c4Index] > 0f)
                        {
                            break;
            ...
            float a = (Img[c1Index] + Img[c2Index]) / 2;
            float b = (Img[c3Index] + Img[c4Index]) / 2;
